...
After the method call a Mobi eBook should be found at the path
you specified in $filename.

=head3 Handling of Images

If your input data ($mhtml) contains <img> tags which are pointing to images on the filesystem, these images will be stored and linked into the MOBI datafile.
The images will be rescaled if necessary, according to L<EBook::MOBI::Picture>.
...
If you stick to the most basic HTML tags it should be perfect mhtml 'compatible'. So if you want to 'write a book' or something similar you can just use basic HTML for markup.
The eBook readers using the MOBI format actually just display plain HTML.
But sadly that's not the whole truth - you can't stick to the official HTML standards.
Since I did some research on how to do things, I'd like to share my knowledge.
...
Hyperlinks pointing to the WWW are working just like in HTML.
But if you want to point into your own file, e.g. for a table of contents, it will not work.
You then have to declare an attribute called 'filepos' which points to the char where you whant to jump to.
...
Images are handled slightly different than in standard HTML, since all the data is not on a normal filesystem - it is packed into the MOBI format.
Since there are no such thing as filenames in the MOBI format (at least as far as I know) you can't point to an image over it's name.
Images are stored in seperat format-intern containers, which have a count.
You can then adress to an image with the number of it's container.
